...
// let result = (array)=>arr.map((item) => item.company.name);
...
//  let result = arr.filter((item)=> item.email.slice(-4) == ".biz");

//  console.log(result);
 
// let result = arr.filter((item) => item.email.endsWith(".biz"))
// console.log(result);
...
// let userCounts = arr.reduce((acc, user) => (acc[user.address.city] = (acc[user.address.city], 0) + 1, acc), {})
...
//  let shuar = arr.reduce((sum,  user) => sum + user.company.catchPhrase.length, 0) 
//  console.log(shuar);


//  let catchPhrase = arr.map((item) => item.company.catchPhrase.length);
